...
CHANGE `purchase_amount_(usd)` purchase_amount DECIMAL(10,2);
...
select item_purchased,avg(review_rating) as 'Average Product Rating' from customer group by item_purchased order by avg(review_rating) desc limit 5; 
 ...
 select subscription_status,count(customer_id) as total_customers ,round(avg(purchase_amount),2) as avg_spend ,round(sum(purchase_amount),2)as total_revenue from customer
...
-- What are the top three most purchased within each category? 
with item_counts as (
...
select subscription_status,count(customer_id) as repest_buyers from customer 
